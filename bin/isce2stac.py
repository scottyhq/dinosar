#!/usr/bin/env python
"""
For a VRT file from ISCE, create STAC-compliant JSON metadata.
...
def cmdLineParse():
    ...
    parser.add_argument('-i', type=str, dest='inFile', required=True,
                        help='input image VRT file')
    parser.add_argument('-l', type=str, dest='isceLog', required=True,
                        help='isce.log file')
    parser.add_argument('-x', type=str, dest='xml', required=False,
                        help='topsApp.xml file')
...
def read_stac_json(stacFile):
    ...
    NOTE: dictionary order only preserved in python > 3.6
...
def get_stac_dictionary(inFile, topsParams, catalog):
    """Create a STAC-compliant python dictionary of metadata.
    ...
    catalogURL = stac_catalog['links'][0]['href']
    ...
    stac_item['assets']['thumbnail']['href'] = '' ## TODO
    stac_item['assets']['thumbnail']['type'] = 'jpeg'

    stac_item['assets']['coherence'] = OrderedDict()
    stac_item['assets']['coherence']['href'] = '' ### TODO
    stac_item['assets']['coherence']['type'] = 'GeoTiff'
    stac_item['assets']['coherence']['cog'] = 'True'
...
def main(parser):
    ...
    topsParams = read_topsAppxml(args.xmlFile)
    isceMeta = read_isce_log(args.isceLog)
    topsParams.update(isceMeta)
    stacDict = get_stac_dictionary(args.inFile, topsParams, args.catalog)
