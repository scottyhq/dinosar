...
def main():
    ...
    infiles = ['coherence-cog.tif', 'amplitude-cog.tif', 'unwrapped-phase-cog.tif']
    for infile in infiles:
        cptFile = dice.make_cmap(infile)
        dout.make_rgb(infile, cptFile, 'tmp.vrt')
        dout.make_overviews('tmp.vrt')
        outfile = infile[:-4] + '-rgb.tif'
        dout.make_cog('tmp.vrt', outfile)

        dout.make_thumbnails(outfile)
    ...
    if not os.path.isdir('output'):
        os.mkdir('output')
    cmd = 'mv index.html ../*log *-cog* output'
    print(cmd)
    subprocess.call(cmd)
